...
class Ferryman {
    ...
    async getIdMeta(oihMeta, self, apiKey) {
        let oihUid = false;
        let refs = false;

        // Check if oihUid exists
        if ('oihUid' in oihMeta && oihMeta.oihUid) {
            const data = await self.getDataHubEntryByOihId(oihMeta.oihUid, apiKey);
            ...
            const data = await self.getDataHubEntryByRecordId(oihMeta.recordUid, apiKey);
    ...
    async processMessage(payload, message) {
        ...
        // Save original incoming passthrough at the very beginning, before any mapping happens
        ...
        try {
            ...
            if (self.amqpConnection) {
                await self.amqpConnection.sendError(e, outgoingMessageHeaders, message);
        ...
        return new Promise((resolve) => {
            ...
            const onData = async (data) => {
                ...
                } else if (!_.isEmpty(origPassthrough)) {
                ...
                if (self.settings.DATAHUB_BASE_URL
                    ...
                    if (dataHubEntryHasRecordUid === false) {
                        ...
                        // Prefers recordUid mapped through transform function
                        // Otherwise fall back on original emitted data
                        const recordUid = (data.metadata && data.metadata.recordUid)
                            ? data.metadata.recordUid : savedMeta.recordUid;
