    ...
    async _getSubscribeChannel() {
        try {
            ...
            if (!this._subscribeChannel) {
                ...
                const subscribeChannelPrefetch = parseInt(process.env.SUBSCRIBE_PREFETCH || '0');
                this._subscribeChannel.prefetch(subscribeChannelPrefetch);
                this._logger.info(`subscribeChannelPrefetch set to ${subscribeChannelPrefetch}`);
            }
