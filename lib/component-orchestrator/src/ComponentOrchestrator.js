...
async function loop(body, logger, loopInterval) {
    ...
    setTimeout(async () => {
        await loop(body, logger, loopInterval);
    }, loopInterval);
}
...
class ComponentOrchestrator {
    ...
    async start() {
        ...
        await loop(this._processState.bind(this), this._logger, this._config.get('TICK_INTERVAL') || 10000);
    }
        ...
        for (let node of nodes) {
            ...
            if (!appsIndex[flow.id] || !appsIndex[flow.id][node.id]) {
                try {
                    ...
                    await this._sendFlowFailedEvent(flow);
                    return;
                }
            }
        }
    ...
    async startComponent(component) {
        ...
        await this._sendComponentStartedEvent(component);
    }
    ...
    async stopComponent(component) {
        ...
        await this._sendComponentStoppedEvent(component);
    }
