...
const StateMachineExecutor = require('./StateMachineExecutor');
...
class ComponentOrchestrator {
    constructor({
        ...
        this._stateMachine = new StateMachineExecutor({
            executionStateDao,
            logger: this._logger.child({ service: 'StateMachineExecutor' })
        });
    }
    ...
    async _handleMessage(message) {
        try {
            ...
            const { isCompleted } = await this._stateMachine.initializeExecution({
                executionId,
                flowExecId,
            ...
            const context = { flow, flowExecId, stepId, messageContent, apiKey };

            try {
                if (routingKey === 'orchestrator_backchannel.step_state') {
                    await this._stateMachine.executeState({
                        executionId,
                        state: 'UPDATING_STEP_STATE',
                        work: async () => {
                    ...
                    await this._stateMachine.completeExecution(executionId);
                    return;
                }
                    ...
                    if (command) {
                        switch (command) {
                            case 'run-next-steps': {
                                ...
                                const results = await this._stateMachine.executeState({
                                    executionId,
                                    state: 'VERIFYING_NEXT_STEPS',
                                    work: async () => {
    ...
    async _executeStep({ flow, flowExecId, stepId, component, apiKey, msg, executionId }) {
        const context = { flow, flowExecId, stepId, component, apiKey, msg };

        await this._stateMachine.executeStep({
            executionId,
            stepId,
            updateCounter: async () => {
                await this._flowStateDao.updateOne(
        ...
        // add webhook specific keys if method exists
                ...
                const config = component.isGlobal
                    ? this._queueCreator.getAmqpGlobalStepConfig(component)
    ...
    async executeFlow({ flow, msg = {} }) {
        ...
        try {
            ...
            const context = { flow: _flow, flowExecId };

            await this._stateMachine.executeState({
                executionId,
                state: 'CREATING_FLOW_STATE',
                work: async () => {
            ...
            this._logger.error({ err }, 'Failed to execute flow');
            throw err;
        }
    }
