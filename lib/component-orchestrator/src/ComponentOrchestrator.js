...
class ComponentOrchestrator {
        ...
        for (let node of nodes) {
            ...
            if (!appsIndex[flow.id] || !appsIndex[flow.id][node.id]) {
                try {
                    ...
                    const envVarsForQueues = await this._queuesManager.getSettingsForNodeExecution(
                        flow,
                        node,
                        flowSettings
                    );

                    const customEnvVars = component.environmentVariables ? {...component.environmentVariables} : {};
                    const envVars = {...envVarsForQueues, ...customEnvVars};
                ...
                if (command) {
                    switch (command) {
                        case 'run-next-steps': {
                            ...
                            // await this._flowStateDao.upsertCount(flowExecId, [results.map(({ targetFlow, targetStepId, componentId }) => `${targetFlow._id}:${targetFlow._id}`], [])

                            this._checkFlowProgress(flowId, stepId, await this._flowStateDao.upsertCount(flowExecId, results.length, 1))
    ...
    async _handleError(message) {
        try {
            ...
            const { stepId, flowId, flowExecId } = jwt.verify(orchestratorToken, this._tokenSecret);

            this._logger.error({ stepId, flowId, flowExecId }, `Step exec failed: ${reboundReason}`);
        } catch (err) {
    ...
    async executeFlow({ flow, msg = {} /*,  msgOpts = {} */ } /* , { type } */) {
        ...
        await this._flowStateDao.upsertCount(flowExecId, 1, 0)
    ...
    async startComponent(component) {
        ...
        const envVarsForQueues = await this._queuesManager.getSettingsForGlobalComponent(
            component,
