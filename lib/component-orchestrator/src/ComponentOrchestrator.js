...
class ComponentOrchestrator {
    ...
    async _handleMessage(message) {
        try {
            ...
            if (!flow) {
                this._logger.warn({ flowId }, 'Flow not found');
                // Don't requeue if flow doesn't exist
                ...
                if (command) {
                    switch (command) {
                        case 'run-next-steps': {
                            ...
                            const stepResults = await Promise.allSettled(results.map(async ({ targetFlow, targetStepId, componentId }) => {
                                try {
                                    const component = await this._componentsDao.findById(componentId);
                                    if (!component) {
                                        throw new Error(`Component ${componentId} not found for step ${targetStepId}`);
                                    }
                                    ...
                                    return { stepId: targetStepId, status: 'success' };
                                } catch (err) {
                                    this._logger.error({ err, flowId: targetFlow._id, flowExecId, stepId: targetStepId },
                            ...
                            const failures = stepResults.filter(r => r.status === 'fulfilled' && r.value.status === 'error');
                            if (failures.length > 0) {
                                this._logger.warn({
                                    flowId: flow._id,
                                    flowExecId,
                                    failures: failures.map(f => f.value)
                                }, `${failures.length} steps failed processing`);
            ...
            if (nextSteps.length) {
                ...
                // Process each step independently
                const results = await Promise.allSettled(nextSteps.map(async (stepId) => {
                    try {
                        const componentId = flow.getPropertiesByNodeId(stepId).componentId;
                        const component = await this._componentsDao.findById(componentId);
                        
                        if (!component) {
                            throw new Error(`Component ${componentId} not found for step ${stepId}`);
                        }
                        ...
                        return { stepId, status: 'success' };
                    } catch (err) {
                        this._logger.error({ err, flowId: flow._id, flowExecId, stepId }, 
                            'Failed to process step');
                        
                        // Increment error counter for the step
                        await this._flowStateDao.updateOne(
                            { flowExecId },
                            { $inc: { errors: 1 } }
                        );
                ...
                if (failures.length > 0) {
                    ...
                    // Return failure but allow requeue
                    return {
                        success: false,
                        error: 'One or more steps failed',
                        requeue: true
            ...
            this._logger.error({ err }, 'Failed to process message');
            
            // Return failure and allow requeue on unexpected errors
            return {
                success: false,
                error: err.message,
                requeue: true
    ...
    async _executeStep({ flow, flowExecId, stepId, component, apiKey, msg }) {
        ...
        const messageId = `${flowExecId}:${message.properties.messageId}`;

        // Use new atomic processStep method
        const shouldProcess = await this._flowStateDao.processStep(flowExecId, stepId, messageId, {
            msg,
            nodeProperties,
            detectedAt: new Date(),
            component: {
        ...
        if (!shouldProcess) {
            this._logger.info({ flowId: flow._id, flowExecId, stepId, messageId }, 'Skipping duplicate message');
            return;
        }
        ...
        // Track message before sending to ensure idempotency
        await this._flowStateDao.updateOne(
            {
                flowExecId,
            },
        ...
        await this._amqp.sendComponentInputMessage(exchangeName, inputRoutingKey, newMessage, {
            ...record,
            headers: { messageId }
        });
    }
    ...
    async executeFlow({ flow, msg = {} /*,  msgOpts = {} */ } /* , { type } */) {
        ...
        // Use upsertState for idempotent flow state creation
        await this._flowStateDao.upsertState(flow._id, flowExecId, flow.tenant);
